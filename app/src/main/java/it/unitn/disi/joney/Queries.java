...
public class Queries {
    ...
    public static String CREATE_TABLE_JOBS  = "CREATE TABLE IF NOT EXISTS Jobs(jobID integer PRIMARY KEY AUTOINCREMENT NOT NULL, title varchar(50) NOT NULL, description varchar(1000), completed boolean NOT NULL, latitude float NOT NULL, longitude float NOT NULL, category integer, authorID integer NOT NULL, workerID integer, FOREIGN KEY (Category) REFERENCES JobCategory(jcID), FOREIGN KEY (authorID) REFERENCES Users(userID), FOREIGN KEY (workerID) REFERENCES User(userID));";
